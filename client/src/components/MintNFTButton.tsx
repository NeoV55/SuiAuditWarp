...
export default function MintNFTButton({ reportUrl, contractName, disabled = false }: MintNFTButtonProps) {
  ...
  const mintNFT = () => {
    ...
    try {
      ...
      const msg = (error as any)?.message || "Failed to prepare NFT minting transaction";
      setMintError(msg);
      toast.error(msg);
      setMintLoading(false);
    }
                ...
                {mintTxResponse.effects?.gasUsed && (
                  <div className="grid grid-cols-3 gap-2">
                    ...
                    <span className="col-span-2 font-mono text-yellow-400">
                      {(mintTxResponse.effects.gasUsed.computationCost || 0) + (mintTxResponse.effects.gasUsed.storageCost || 0)} MIST
                    </span>
                  </div>
                )}
