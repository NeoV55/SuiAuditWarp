...
import logo from "../../../attached_assets/logo-text-removebg-preview_1748011262574.png";
...
function parseAuditResults(auditText: string) {
  ...
  const sentences = cleanText.split(/[.!?]\s+/).filter(sentence => sentence.trim().length > 10);
  ...
  for (const sentence of sentences) {
    ...
    if ((lowerSentence.includes('summary') || lowerSentence.includes('overview') || 
         lowerSentence.includes('audit found') || lowerSentence.includes('analysis reveals')) && !summaryFound) {
      sections.summary = sentence.trim() + '.';
      summaryFound = true;
    }
    // Look for vulnerabilities and security issues
    else if (lowerSentence.includes('vulnerability') || lowerSentence.includes('security') || 
             lowerSentence.includes('risk') || lowerSentence.includes('issue') || 
             lowerSentence.includes('concern') || lowerSentence.includes('flaw')) {
      sections.vulnerabilities.push(sentence.trim() + '.');
    }
    // Look for recommendations
    else if (lowerSentence.includes('recommend') || lowerSentence.includes('suggest') || 
             lowerSentence.includes('should') || lowerSentence.includes('consider') ||
             lowerSentence.includes('implement') || lowerSentence.includes('improve')) {
      sections.recommendations.push(sentence.trim() + '.');
    }
    // Look for scores or ratings
    else if (lowerSentence.includes('score') || lowerSentence.includes('rating') || 
             lowerSentence.includes('severity') || lowerSentence.includes('level')) {
  ...
  if (!sections.summary && sections.vulnerabilities.length === 0 && sections.recommendations.length === 0) {
  ...
  if (!sections.summary) {
    sections.summary = 'This audit analysis provides a comprehensive security assessment of the smart contract code.';
  }
  
  if (sections.vulnerabilities.length === 0) {
    sections.vulnerabilities.push('No critical vulnerabilities detected in the initial automated scan.');
  }
  
  if (sections.recommendations.length === 0) {
    sections.recommendations.push('Continue with thorough manual review and testing.');
    sections.recommendations.push('Implement comprehensive unit tests for all contract functions.');
  }
...
export async function generatePDF(
  ...
  try {
    ...
    const addSection = (page: any, title: string, content: string | string[], isArray = false) => {
    ...
    if (auditSections.summary) {
      currentPage = addSection(currentPage, "EXECUTIVE SUMMARY", auditSections.summary);
    }
    
    if (auditSections.security_score) {
      currentPage = addSection(currentPage, "SECURITY ASSESSMENT", auditSections.security_score);
    }
    
    if (auditSections.vulnerabilities.length > 0) {
      currentPage = addSection(currentPage, "IDENTIFIED VULNERABILITIES", auditSections.vulnerabilities, true);
    }
    
    if (auditSections.recommendations.length > 0) {
      currentPage = addSection(currentPage, "RECOMMENDATIONS", auditSections.recommendations, true);
    }
    
    if (auditSections.details) {
      currentPage = addSection(currentPage, "DETAILED ANALYSIS", auditSections.details);
    }
    ...
    const codeExcerpt = cleanCode.length > 1000 
    ...
    if (contractCode.length > 1000) {
      codePage.drawText("[Code excerpt truncated for PDF rendering - full code analyzed in audit]", {
        x: 50,
        y: codeY - 20,
        size: 9,
        font: helveticaFont,
